...
export default function ProjectCard({ project, className = "", onClick }: ProjectCardProps) {
  return (
    <div
      onClick={onClick}
      className={`proj-card bg-white border border-[#e4e0da] rounded-lg overflow-hidden flex flex-col transition-all duration-[350ms] hover:-translate-y-1.5 hover:shadow-[0_24px_64px_rgba(0,0,0,0.09)] ${onClick ? "cursor-pointer" : ""} ${className}`}
    >
      {/* Thumbnail */}
      <div className={`h-[210px] relative overflow-hidden flex items-center justify-center bg-gradient-to-br ${project.thumbClass}`}>
        ...
        {/* Hover overlay hint */}
        {onClick && (
          <div className="absolute inset-0 bg-black/0 hover:bg-black/10 transition-colors duration-300 flex items-end justify-end p-3">
            <span className="opacity-0 group-hover:opacity-100 bg-white/90 text-[#0e0e0e] text-[11px] font-semibold px-2.5 py-1 rounded-full">
              View Details →
            </span>
          </div>
        )}
