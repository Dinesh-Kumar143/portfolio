    ...
    >
      ...
      <div className="max-w-[1200px] mx-auto px-13 w-full relative z-10">
        <div className="grid grid-cols-[1fr_400px] gap-16 items-center max-lg:grid-cols-1">
          ...
          <div>
            <motion.div
              className="inline-flex items-center gap-2.5 text-[12px] font-medium tracking-[0.1em] uppercase text-[#6b6b6b] mb-6"
              {...fadeUp(0.1)}
            >
            ...
            <motion.h1
              className="font-serif text-[clamp(46px,5.5vw,80px)] font-bold leading-[1.04] tracking-[-0.025em] text-[#0e0e0e] mb-6"
              {...fadeUp(0.2)}
            >
              Building{" "}<em className="text-[#c8622a] italic">digital</em>
              <br />{personalInfo.tagline}
            </motion.h1>

            <motion.p
              className="text-[16px] text-[#3a3a3a] leading-[1.8] max-w-[500px] mb-10"
              {...fadeUp(0.3)}
            >
              {personalInfo.bio}
            </motion.p>

            <motion.div className="flex items-center gap-3.5 flex-wrap mb-13" {...fadeUp(0.4)}>
            ...
            <motion.div
              className="flex gap-10 pt-10 border-t border-[#e4e0da] max-sm:flex-col max-sm:gap-5"
              {...fadeUp(0.5)}
            >
              {personalInfo.stats.map((stat) => (
                <div key={stat.label}>
                  <span className="font-serif text-[42px] font-bold text-[#0e0e0e] leading-none block">
                    {stat.num.replace("+", "")}
                    {stat.num.includes("+") && <em className="not-italic text-[#c8622a]">+</em>}
                  </span>
                  <p className="text-[12px] text-[#6b6b6b] mt-1 font-medium tracking-[0.04em]">{stat.label}</p>
                </div>
              ))}
            </motion.div>
          </div>
          ...
          >
            <div className="bg-white border border-[#e4e0da] rounded-lg overflow-hidden shadow-[0_24px_80px_rgba(0,0,0,0.09),0_4px_16px_rgba(0,0,0,0.04)]">
              <div className="bg-[#0e0e0e] px-6 py-5 flex items-center gap-3.5">
                ...
                <div>
                  <span className="block text-[15px] font-semibold text-white">{personalInfo.name}</span>
                  <span className="block text-[12px] text-white/45 mt-0.5">{personalInfo.title}</span>
                </div>
              ...
              <div>
                {[
                  ...
                  <div key={row.label} className="flex items-center gap-3.5 px-6 py-3.5 border-b border-[#e4e0da] last:border-b-0">
                    ...
                    <div>
                      <span className="block text-[11px] text-[#6b6b6b] font-medium mb-0.5">{row.label}</span>
                      <span className="block text-[13px] font-medium text-[#0e0e0e]">{row.value}</span>
                    </div>
                  </div>
                ))}
              </div>
